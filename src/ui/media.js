...
const renderers = {
  image: media => `<figure class="content-media"><img src="${e(safeUrl(media.src,true))}" alt="${e(media.alt)}" loading="lazy"><figcaption>${e(media.caption || media.alt)} ${media.credit?`· ${e(media.credit)}`:''}${safeUrl(media.sourceUrl)?` · <a href="${e(safeUrl(media.sourceUrl))}" target="_blank" rel="noopener">来源与授权 ↗</a>`:''}</figcaption></figure>`,
...
export function renderMedia(items = [], depth) {
  return items.filter(m=>!m.depths || m.depths.includes(depth)).map(media=> {
    if (!renderers[media.type] || !safeUrl(media.src,media.type!=='resource')) return '';
